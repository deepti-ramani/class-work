...
void test7(int max_length)
{
  ...
  */
  file_result = word_lengths_efficient(dictionary, lengths, max_length);
  if (file_result == FILE_ERR_OPEN)
  {
    printf("Can't open %s\n", dictionary);
    return;
...
void test8(const char* dictionary, int deviation)
{
	int file_result;
	int i;
	
	const char *words[] = { "apple", "appl3", "apl", "bnaa", "passionfruit", "kiiiwis", "aaapppllleee", "vegetable" };
	...
	for(i = 0; i < num_words; i++)
	{
		file_result = closest_word(dictionary, words[i], deviation);
		if (file_result == FILE_ERR_OPEN)
		{
		  printf("Can't open %s\n", dictionary);
		  return;
